...
export default function QuotationViewForm({ currentQuotation }) {
  ...
  const NewQuotationSchema = Yup.object().shape({
    ...
    password: !currentQuotation
      ? Yup.string()
          .min(6, 'Password must be at least 6 characters')
          .required('Password is required')
      : Yup.string(),
    confirmPassword: !currentQuotation
      ? Yup.string()
  ...
  const defaultValues = useMemo(
    () => ({
      ...
      employeeId: currentQuotation?.employeeId || '',
      email: currentQuotation?.email || '',
      isActive: currentQuotation?.isActive || true,
      avatarUrl: currentQuotation?.avatar?.fileUrl || null,
